...
if [ "x${SRC_PKGNAME}" == "x${DEST_PKGNAME}" ]; then
	if [ ${TEST_MODE} == 1 ]; then
		echo "mv storehouse/${SRC_DIR} storehouse/${DEST_DIR}"
	else
		mv storehouse/${SRC_DIR} storehouse/${DEST_DIR}
	fi
else
	if [ ${TEST_MODE} == 1 ]; then
		echo "mv storehouse/${SRC_DIR} storehouse/${DEST_DIR}"
		...
		do
			echo "mv ${i} storehouse/${DEST_DIR}/"$(basename ${i} | sed -e "s@^${SRC_PKGNAME}\$@${DEST_PKGNAME}@g" -e "s@^${SRC_PKGNAME}\.@${DEST_PKGNAME}\.@g") | sed "s@storehouse/${SRC_DIR}@storehouse/${DEST_DIR}@g"
		done
	else
		mv storehouse/${SRC_DIR} storehouse/${DEST_DIR}
